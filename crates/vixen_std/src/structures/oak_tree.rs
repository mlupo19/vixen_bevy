    ...
    fn generate(&self, position: BlockCoord, in_progress: Arc<DashMap<ChunkCoord, UnfinishedChunkData>>, rng: &mut rand::rngs::StdRng) {
        ...
        for y in 1..OAK_LEAVES_HEIGHT-1 {
            for x in 0..OAK_LEAVES_RADIUS {
                for z in 0..OAK_LEAVES_RADIUS {
                    ...
                    set_block_in_neighborhood(position + ivec3(x, leaves_height + y, z), block, in_progress.clone());
                    set_block_in_neighborhood(position + ivec3(-x, leaves_height + y, z), block, in_progress.clone());
                    set_block_in_neighborhood(position + ivec3(x, leaves_height + y, -z), block, in_progress.clone());
                    set_block_in_neighborhood(position + ivec3(-x, leaves_height + y, -z), block, in_progress.clone());
                }
        ...
        for x in 0..OAK_LEAVES_RADIUS-1 {
            for z in 0..OAK_LEAVES_RADIUS-1 {
                ...
                set_block_in_neighborhood(position + ivec3(x, leaves_height, z), block, in_progress.clone());
                set_block_in_neighborhood(position + ivec3(-x, leaves_height, z), block, in_progress.clone());
                set_block_in_neighborhood(position + ivec3(x, leaves_height, -z), block, in_progress.clone());
                set_block_in_neighborhood(position + ivec3(-x, leaves_height, -z), block, in_progress.clone());

                set_block_in_neighborhood(position + ivec3(x, leaves_height + OAK_LEAVES_HEIGHT - 1, z), block, in_progress.clone());
                set_block_in_neighborhood(position + ivec3(-x, leaves_height + OAK_LEAVES_HEIGHT - 1, z), block, in_progress.clone());
                set_block_in_neighborhood(position + ivec3(x, leaves_height + OAK_LEAVES_HEIGHT - 1, -z), block, in_progress.clone());
                set_block_in_neighborhood(position + ivec3(-x, leaves_height + OAK_LEAVES_HEIGHT - 1, -z), block, in_progress.clone());
            }
