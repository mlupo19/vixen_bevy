...
use noise::{core::worley::ReturnType, NoiseFn, Fbm, Perlin, Curve, MultiFractal, Clamp, Min, ScaleBias, Turbulence, Select, Seedable, Terrace, RidgedMulti, Constant, Blend, Max, Multiply, Add, Exponent, Billow, Worley, Cache};
...
// elevations.
//
pub fn baseContinentDef(seed: u32) -> impl NoiseFn<f64, 3> {
...
pub fn complex_noise_planet(seed: u32) -> impl NoiseFn<f64, 3> + Send + Sync {
...
//                               100);
    ...
    // detail to it.
    let continentDef_tu0 = Turbulence::<_, Perlin>::new(baseContinentDef(seed))
        .set_seed(seed + 10)
    ...
    // remain unaffected.
    let continentDef_se = Select::new(baseContinentDef(seed), continentDef_tu2, baseContinentDef(seed))
    ...
    let continentDef = NoiseWrapper::new(Cache::new(continentDef_se));
    ...
    // fjords.
    let terrainTypeDef_tu = Turbulence::<_, Perlin>::new(continentDef.clone())
        .set_seed(seed + 20)
    ...
    let terrainTypeDef = NoiseWrapper::new(Cache::new(terrainTypeDef_te));
    ...
    let mountainBaseDef = NoiseWrapper::new(Cache::new(mountainBaseDef_tu1));
    ...
    let mountainousTerrain_ad = Add::new(mountainousTerrain_sb1, mountainBaseDef.clone());
    ...
    // the ocean.
    let continentalShelf_te = Terrace::new(continentDef.clone())
    ...
    // is measured in planetary elevation units.
    let baseContinentElev_sb = ScaleBias::new(continentDef.clone())
    ...
    // module.
    let baseContinentElev_se = Select::new(baseContinentElev_sb, continentalShelf, continentDef.clone())
    ...
    let baseContinentElev = NoiseWrapper::new(Cache::new(baseContinentElev_se));
    ...
    let continentsWithPlains_ad = Add::new(baseContinentElev.clone(), scaledPlainsTerrain);

    // 2: [Continents-with-plains subgroup]: Caches the output value from the
    // continents-with-plains module.
    let continentsWithPlains = NoiseWrapper::new(Cache::new(continentsWithPlains_ad));
    ...
    let continentsWithHills_ad = NoiseWrapper::new(Add::new(baseContinentElev.clone(), scaledHillyTerrain));
    ...
    let continentsWithHills_se = Select::new(
        continentsWithPlains.clone(),
        continentsWithHills_ad.clone(),
        terrainTypeDef.clone(),
    )
    ...
    // subgroup.
    let continentsWithMountains_ad0 = Add::new(baseContinentElev.clone(), scaledMountainousTerrain);
    ...
    let continentsWithMountains_cu = Curve::new(continentDef.clone())
    ...
    // select-high-elevations module.
    let continentsWithMountains = NoiseWrapper::new(Cache::new(continentsWithMountains_se));
    ...
    // subgroup.
    let continentsWithBadlands_ad = Add::new(baseContinentElev.clone(), scaledBadlandsTerrain);
    ...
    let continentsWithBadlands_ma = Max::new(continentsWithMountains.clone(), continentsWithBadlands_se);

    // 5: [Continents-with-badlands subgroup]: Caches the output value from the
    //    apply-badlands module.
    let continentsWithBadlands = NoiseWrapper::new(Cache::new(continentsWithBadlands_ma));
    ...
    let continentsWithRivers_ad = Add::new(continentsWithBadlands.clone(), continentsWithRivers_sb);
...
impl<Source: NoiseFn<f64, DIM>, const DIM: usize> NoiseFn<f64, DIM> for NoiseWrapper<DIM, Source>
{
    fn get(&self, point: [f64; DIM]) -> f64 {
        self.0.as_ref().get(point)
    }
}

struct NoiseWrapper<const DIM: usize, T: NoiseFn<f64, DIM>>(pub Arc<T>);

impl<Source: NoiseFn<f64, DIM>, const DIM: usize> NoiseWrapper<DIM, Source> {
    pub fn new(noise: Source) -> Self {
        Self(Arc::new(noise))
    }
}

impl<Source: NoiseFn<f64, DIM>, const DIM: usize> Clone for NoiseWrapper<DIM, Source> {
    fn clone(&self) -> Self {
        Self(self.0.clone())
    }
}