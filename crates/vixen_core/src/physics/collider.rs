...
impl AABB {
    ...
    pub fn from_player(position: Vec3) -> Self {
        let min = Vec3::new(
            position.x - PLAYER_SIZE.0 / 2.,
            position.y,
            position.z - PLAYER_SIZE.2 / 2.,
        );
        let max = Vec3::new(
            position.x + PLAYER_SIZE.0 / 2.,
            position.y + PLAYER_SIZE.1,
            position.z + PLAYER_SIZE.2 / 2.,
        );
        AABB::new(min, max)
    }
    ...
    pub fn extend(&self, extension: &Vec3) -> Self {
        let (min, max) = (
            self.min.min(self.min + *extension),
            self.max.max(self.max + *extension),
        );
        AABB::new(min, max)
    }
}
...
impl SweptCollider<AABB> for AABB {
    fn swept_collide(&self, other: &AABB, movement: &mut Movement) -> Option<(f32, Vec3)> {
        ...
        let tx = collide_time_axis(
            Axis {
                min: self.min.x,
                max: self.max.x,
                velo: movement.delta.x,
            },
            Axis {
                min: other.min.x,
                max: other.max.x,
        ...
        let ty = collide_time_axis(
            Axis {
                min: self.min.y,
                max: self.max.y,
                velo: movement.delta.y,
            },
            Axis {
                min: other.min.y,
                max: other.max.y,
        ...
        let tz = collide_time_axis(
            Axis {
                min: self.min.z,
                max: self.max.z,
                velo: movement.delta.z,
            },
            Axis {
                min: other.min.z,
                max: other.max.z,
            ...
            || tz.0 > 1.0
        {
            ...
            _normal = match entry_time {
                t if t == tx.0 => {
                    tangent.x = 0.0;
                    vec3(-1.0, 0.0, 0.0) * tx.2.signum()
                }
                t if t == ty.0 => {
                    tangent.y = 0.0;
                    vec3(0.0, -1.0, 0.0) * ty.2.signum()
                }
                t if t == tz.0 => {
                    tangent.z = 0.0;
                    vec3(0.0, 0.0, -1.0) * tz.2.signum()
                }
...
impl BroadphaseCollider<AABB> for AABB {
    ...
    fn might_collide(&self, other: &AABB, movement: &Movement) -> bool {
        let extended_aabb = AABB::new(
            self.min.min(self.min + movement.delta),
            self.max.max(self.max + movement.delta),
        );
...
mod tests {
    ...
    use super::{Movement, SweptCollider, AABB};
